...
async function fetchAlphaStock(symbol: string): Promise<MarketPoint | null> {
  ...
  const url = `${ALPHA_VANTAGE_URL}?function=TIME_SERIES_DAILY&symbol=${encodeURIComponent(symbol)}&outputsize=compact&apikey=${encodeURIComponent(alphaKey)}`;
  ...
  return {
    symbol, name: symbol, type: "STOCK", price,
    previousPrice: Number.isFinite(previousPrice) ? previousPrice : price,
    currency: "USD", externalSymbol: symbol, source: "alpha_vantage",
    timestamp: new Date(`${dates[0]}T23:59:59Z`),
  };
}

async function fetchYahooStock(symbol: string): Promise<MarketPoint | null> {
  const url = `${YAHOO_CHART_URL}/${encodeURIComponent(symbol)}?range=1d&interval=1m&events=quote`;
  const data = await jsonFetch(url);
...
async function fetchAlphaForex(from: string, to: string): Promise<MarketPoint | null> {
  if (!alphaKey) return null;
  const url = `${ALPHA_VANTAGE_URL}?function=CURRENCY_EXCHANGE_RATE&from_currency=${from}&to_currency=${to}&apikey=${encodeURIComponent(alphaKey)}`;
  const data = await jsonFetch(url);
  ...
  if (!Number.isFinite(price)) return null;
  return {
    symbol: `${from}/${to}`, name: `${from} to ${to}`, type: "FOREX",
    price, previousPrice: price, currency: to, externalSymbol: `${from}${to}`,
    source: "alpha_vantage", timestamp: new Date(),
  };
}
...
async function fetchPublicForex(): Promise<MarketPoint | null> {
  ...
  if (!Number.isFinite(price)) return null;
  return {
    symbol: "USD/NGN", name: "US Dollar to Nigerian Naira", type: "FOREX",
    price, previousPrice: price, currency: "NGN", externalSymbol: "USDNGN",
    source: "exchange_rate_api", timestamp: new Date(),
  };
}

async function fetchCoinGecko(): Promise<MarketPoint[]> {
  const keyParam = coinGeckoKey ? `&x_cg_demo_api_key=${encodeURIComponent(coinGeckoKey)}` : "";
  const url = `${COINGECKO_URL}/coins/markets?vs_currency=usd&ids=bitcoin,ethereum&price_change_percentage=24h${keyParam}`;
  ...
  return data.filter((coin) => Number.isFinite(coin.current_price)).map((coin) => {
    ...
    const previousPrice =
      Number.isFinite(change) && change !== -100
        ...
        : coin.current_price;
    return {
      symbol: coin.symbol.toUpperCase(), name: coin.name, type: "CRYPTO" as const,
      price: coin.current_price, previousPrice, currency: "USD",
      externalSymbol: coin.symbol.toUpperCase(), externalId: coin.id,
      marketCap: coin.market_cap ?? undefined, volume24h: coin.total_volume ?? undefined,
      source: "coingecko", timestamp: new Date(),
    };
  });
}
...
async function saveMarketPoint(point: MarketPoint) {
  ...
  const previousPrice = existing && point.previousPrice === point.price ? Number(existing.price) : point.previousPrice;

  const asset = await prisma.asset.upsert({
    where: { symbol: point.symbol },
    update: {
      name: point.name, type: point.type, previousPrice, price: point.price,
      externalSymbol: point.externalSymbol, externalId: point.externalId,
      currency: point.currency, marketCap: point.marketCap, volume24h: point.volume24h,
      dataSource: point.source, lastFetchedAt: new Date(), isActive: true,
    },
    create: {
      symbol: point.symbol, name: point.name, type: point.type, price: point.price,
      previousPrice: point.previousPrice, externalSymbol: point.externalSymbol,
      externalId: point.externalId, currency: point.currency, marketCap: point.marketCap,
      volume24h: point.volume24h, dataSource: point.source, lastFetchedAt: new Date(), isActive: true,
    },
  });

  await prisma.priceCandle.create({
    data: {
      assetId: asset.id, timestamp: point.timestamp, open: point.price,
      high: point.price, low: point.price, close: point.price, dataSource: point.source,
    },
  });
...
async function snapshotPortfolios() {
  ...
  for (const user of users) {
    const [portfolio, investments, trades] = await Promise.all([
      ...
      prisma.userInvestment.findMany({ where: { userId: user.id, status: "ACTIVE" } }),
      prisma.paperTrade.findMany({ where: { userId: user.id, status: "OPEN" }, include: { asset: true } }),
    ]);
    ...
    const tradeValue = trades.reduce((sum, trade) => sum + Number(trade.quantity) * Number(trade.asset.price), 0);
...
export async function syncMarketData() {
  ...
  for (const symbol of stocks) {
    try {
      let point: MarketPoint | null = null;

      if (alphaKey) {
        try {
          point = await fetchAlphaStock(symbol);
        } catch (error) {
      ...
      if (!point) {
        try {
          point = await fetchYahooStock(symbol);
        } catch (error) {
          errors.push(`Yahoo Finance ${symbol}: ${error instanceof Error ? error.message : "unknown error"}`);
        }
      }
  ...
  try {
    let forex: MarketPoint | null = null;

    if (alphaKey) {
      try {
        forex = await fetchAlphaForex("USD", "NGN");
      } catch (error) {
    ...
    if (!forex) {
      try {
        forex = await fetchPublicForex();
      } catch (error) {
        errors.push(`ExchangeRate API USD/NGN: ${error instanceof Error ? error.message : "unknown error"}`);
      }
    }
  ...
  for (const point of points) await saveMarketPoint(point);
